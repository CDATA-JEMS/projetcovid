--Load data Format 1
csv_lines_covid_format_1 = LOAD '/user/cloudera/datalake/prepared_data/covid/format1' USING PigStorage(',','-tagFile') AS (
			filename:chararray,
			state:chararray,
			country:chararray,
			date:datetime,
...
--Load data Format 2
csv_lines_covid_format_2 = LOAD '/user/cloudera/datalake/prepared_data/covid/format2' USING PigStorage(',','-tagFile') AS (
			filename:chararray,
			fips:int,
			admin2:chararray,
			state:chararray,
...
--Save the results
STORE merged_dataset into '/user/cloudera/datalake/prepared_data/covid_19_complete' using PigStorage(';');

STORE output_etl_country_per_day_covid into '/user/cloudera/datalake/prepared_data/covid_19_country_per_day' using PigStorage(';');
